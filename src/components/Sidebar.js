...
const SidebarCategory = withRouter(
  ({ name, badgeColor, badgeText, icon, isOpen, children, onClick, location, to }) => {
    ...
    return (
      <li className={"sidebar-item " + getSidebarItemClass(to)}>
        <span data-toggle="collapse" className={"sidebar-link " + (!isOpen?"collapsed":"")} 
          onClick={onClick} aria-expanded={isOpen?"true":"false"} >
        ...
        <Collapse isOpen={isOpen}>
          <ul id="item" className={"sidebar-dropdown list-unstyled"}>{children}</ul>
        </Collapse>
      </li>
    );
  }
...
const Sidebar = ({ location, sidebar }) => {
  ...
  return (
    <nav
      className={classNames("sidebar", sidebar.isOpen || "toggled")}
    >
      <div className="sidebar-content">
        <a className={classNames("sidebar-brand")} href="/dashboard" >
          <img src={brandUBU} alt="Science Park UBU" width="35" className="align-middle mr-2" />{" "}
          <span className="align-middle">UBU Spark</span>{" "}
        </a>
          ...
          {routes.map((category, index) => {
            return (
              <React.Fragment key={index}>
                ...
                {category.children?(
                  <SidebarCategory
                    name={category.name} badgeColor={category.badgeColor} badgeText={category.badgeText}
                    icon={category.icon} to={category.path} isOpen={openRoutes[index]}
...
export default withRouter(
  connect(store => ({ sidebar: store.sidebar }))(Sidebar)
);
